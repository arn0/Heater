...
/*
 * @brief Write update target temperature to file
 * @param Updated target temperature
 */
...
/*
 * @brief Read stored target temperature from file
 * @param Pointer to target temperature
 */
...
void control_task() {
	...
	do {
		...
		if ( heater_status.bck > MAX_INTERNAL_TEMP ) {
			heater_status.one_on = false;
			heater_status.two_on = false;
		}
...
bool start_control_task() {
	...
	xTaskCreate( control_task, "control_task", 4096/2, NULL, CONTROL_TASK_PRIORITY, NULL );
